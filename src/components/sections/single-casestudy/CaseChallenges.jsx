...
export default function CaseChallenges({ data }) {
  if (!data) return null;

  const { sub_heading, heading, content_heading, short_text, challenge_image } = data;
  const imgUrl = challenge_image?.url || challenge_image?.sizes?.large;

  return (
    <section className="bg-(--color-dark) overflow-hidden">
      <div className="web-width mx-auto px-6 py-20 md:py-32">
        ...
        >
          ...
          {heading && (
            <motion.h2
              variants={fadeUp}
              className="text-3xl md:text-4xl font-bold text-white leading-[1.08] max-w-2xl"
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-20 items-center">
          ...
          >
            {content_heading && (
              <motion.h3 variants={fadeUp} className="text-xl font-semibold text-white mb-5">
                {content_heading}
              </motion.h3>
            )}
            {short_text && (
              <motion.div
                variants={fadeUp}
                className="
                  prose prose-invert max-w-none text-white/60 text-base leading-relaxed
                  [&_p]:mb-5
          ...
          {/* Image — clip-path wipe reveal from bottom */}
          {imgUrl && (
            <motion.div
            ...
            >
              <Image
                src={imgUrl}
                alt={challenge_image?.alt || content_heading || "Challenge"}
                fill
                sizes="(max-width: 1024px) 100vw, 50vw"
                className="object-cover"
              />
              <div className="absolute inset-y-0 left-0 w-1 bg-(--color-accent)" />
              <div className="absolute inset-0 bg-(--color-dark)/20" />
            </motion.div>
          )}
        </div>
      </div>
