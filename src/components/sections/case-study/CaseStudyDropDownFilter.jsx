        ...
        <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-6 mb-10 md:mb-14">
          ...
          {!loading && filters.length > 1 && (
            <div className="relative shrink-0" ref={dropdownRef}>
              <button
                onClick={() => setOpen((v) => !v)}
                className="flex items-center gap-3 min-w-48 px-5 py-3 bg-white border border-[#1E1E1E]/20 rounded-lg text-sm font-medium text-[#1E1E1E] hover:border-[#1E1E1E] transition-colors duration-200"
              >
              ...
              {open && (
                <ul className="absolute right-0 z-20 mt-2 w-full min-w-48 bg-white border border-[#1E1E1E]/10 rounded-lg shadow-lg overflow-hidden">
                  {filters.map((f) => (
                    <li key={f}>
                      <button
                        onClick={() => { setActiveFilter(f); setOpen(false); }}
